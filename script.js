...
function getStockPrices() {
    ...
    fetch(`https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=${choice}&apikey=G0FXED0O9KX71PZX`)
        ...
        console.log(JSON.stringify(Object.values(data["Weekly Time Series"])[0]))
        for (i = 0; i < document.getElementsByTagName('li').length; i++) {
            document.querySelectorAll('li')[i].innerHTML = JSON.stringify(Object.values(data["Weekly Time Series"])[i])
        }
  })
  .catch(error => {
